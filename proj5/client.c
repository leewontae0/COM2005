...
/*
 * 스레드풀이 새 작업을 받지 않을 때까지 쉬지 않고 작업을 넣는다.
 */
void bar1(void *arg)
{
    pthread_pool_t *pool = (pthread_pool_t *)arg;
    
    while (pthread_pool_submit(pool, dot, NULL, POOL_WAIT) != POOL_FAIL)
        ;
}

/*
 * 스레드풀에 제한된 수만큼 작업을 넣는다. 스레드풀이 새 작업을 받지 않으면 즉시 중단한다.
 */
void bar2(void *arg)
{
    pthread_pool_t *pool = (pthread_pool_t *)arg;
    ...
    for (int i = 0; i < 2048; ++i)
        if (pthread_pool_submit(pool, dot, NULL, POOL_WAIT) == POOL_FAIL)
            break;
}
...
int main(void)
{
    ...
    printf("소수 %d개를 찾았다.\n", count);
    if (count == 18)
        printf("소수 18개를 모두 찾았다.\n");
    else if (count > 18) {
        printf("Error: POOL_DISCARD 옵션 오류\n");
        return -1;
    }
    else if (count == 0)
        printf("일꾼 스레드가 하나도 구동되지 않았다. 드물지만 이런 현상이 나타날 수 있다.\n그러나 지속적으로 0이 나온다면 알고리즘을 수정할 필요가 있다.\n");
    else if (count < 18)
        printf("일부 일꾼 스레드가 구동되기 전에 풀이 종료되었다. 오류는 아니다.\n");
    ...
    printf("......PASSED\n");
    /*
     * 여러 개의 스레드풀이 서로 접근하는 동안 동기화가 잘 되는지 검증한다.
     * 스레드풀에 있는 작업이 다른 스레드풀에 작업을 넣기도 하고, 
     * 같은 스레드풀에 여러 개의 스레드가 동시에 작업을 넣으려고도 한다.
     */
    printf("--- 교차 검증 ---\n");
    pthread_pool_init(&pool4, 3, 10);
    for (i = 0; i < NLOOP; ++i) {
        /*
         * 용량이 한정된 스레드풀 세 개를 생성한다.
         */
        pthread_pool_init(&pool1, 8, 16);
        pthread_pool_init(&pool2, 8, 16);
        pthread_pool_init(&pool3, 8, 16);
        /*
         * 각 스레드풀에 6개의 작업을 요청한다.
         * 이 6개의 작업은 다른 스레드풀에 또 다른 작업을 요청한다.
         */ 
        for (int j = 0; j < 6; ++j) {
            pthread_pool_submit(&pool1, bar1, &pool2, POOL_WAIT);
            pthread_pool_submit(&pool2, bar1, &pool3, POOL_WAIT);
            pthread_pool_submit(&pool3, bar2, &pool1, POOL_WAIT);
        }
        /*
         * 앞에서 만들었던 스레드풀 4번은 지속적으로 사용한다.
         * 나머지 스레드풀을 종료한다. 종료 옵션은 무작위로 선택한다.
         */
        pthread_pool_submit(&pool4, number2, &i, POOL_NOWAIT);
        pthread_pool_shutdown(&pool3, rand()%2?POOL_COMPLETE:POOL_DISCARD);
        pthread_pool_shutdown(&pool2, rand()%2?POOL_COMPLETE:POOL_DISCARD);
        pthread_pool_shutdown(&pool1, rand()%2?POOL_COMPLETE:POOL_DISCARD);
    }
    /*
     * 마지막으로 스레드풀 4번을 종료하고 마친다.
     */
    pthread_pool_shutdown(&pool4, POOL_COMPLETE);
    printf("......PASSED\n");
    /*
     * 총 실행시간을 계산한다.
     */
