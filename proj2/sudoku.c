...
/* 서브그리드 좌측 상단의 좌표를 직관적으로 볼 수 있게 하기 위해 만든 구조체 */
typedef struct ROW_COL
{
    int r; /* 서브그리드 좌측 상단 행의 값 */
    int c; /* 서브그리드 좌측 상단 열의 값 */
};
 ...
 * 행 번호는 0부터 시작하며, i번 행이 올바르면 valid[0][y]에 true를 기록한다.
 */
void *check_rows(void *arg)
{
    int cnt[10]; /* 행에서 1에서 9까지의 값이 하나씩만 나왔는지 확인하기 위한 배열 */
    ...
    for (int y = 0; y < 9; ++y) {
        ...
        for (int x = 0; x < 9; ++x) {
            /* 
             * 배열에서 인덱스가 스도쿠 값인 인자의 값을 1 증가시킨다
             * 만약 증가되기 전 값이 0이라면, 중복되지 않은 값이고
             * 1이라면, 중복된 값이다.
             * 중복된 값이라면 해당 행은 올바르지 않은 행이므로 더 이상의 검사는 불필요하니 반복문을 나간다.
             */
            if (cnt[sudoku[y][x]] == 0) ++cnt[sudoku[y][x]];
            else {
                valid[0][y] = false;
                break;
            }
 ...
 * 열 번호는 0부터 시작하며, j번 열이 올바르면 valid[1][x]에 true를 기록한다.
 */
void *check_columns(void *arg)
{
    ...
    for (int x = 0; x < 9; ++x) {
        ...
        for (int y = 0; y < 9; ++y) {
             ...
             * 만약 증가되기 전 값이 0이라면, 중복되지 않은 값이고
             * 1이라면, 중복된 값이다.
             * 중복된 값이라면 해당 열은 올바르지 않은 행이므로 더 이상의 검사는 불필요하니 반복문을 나간다.
             */
            if (cnt[sudoku[y][x]] == 0) ++cnt[sudoku[y][x]];
            else {
                valid[1][x] = false;
...
{
    // 여기를 완성하세요
    /* 입력받은 인자를 통해, 서브그리드의 좌측상단 행과 열을 입력받는다. */
    struct ROW_COL pos = *((struct ROW_COL*) arg);
    int r = pos.r; /* 서브그리드 좌측상단의 행의 값 */
    int c = pos.c; /* 서브그리드 좌측상단의 열의 값 */
    int k = (c / 3) * 3 + (r / 3); /* 서브그리드의 순서를 맞추기 위해, 입력받은 행과 열의 값을 통해 값을 설정 */
    bool con = true; /* 서브그리드가 올바른지 확인하기 위한 변수 */

    int cnt[10]; /* 행에서 1에서 9까지의 값이 하나씩만 나왔는지 확인하기 위한 배열 */
    memset(cnt, 0, 10 * sizeof(int));
    /* 
     * 서브그리드 내에서 검사
     * 입력받은 행과 열의 값에서, 각각 0에서 2까지 증가시켜 모든 서브그리드 내의 인자를 검사
     */
    for (int y = 0; y < 3; ++y) {
        for (int x = 0; x < 3; ++x) {
            /* 
             ...
             * 만약 증가되기 전 값이 0이라면, 중복되지 않은 값이고
             * 1이라면, 중복된 값이다.
             * 중복된 값이라면 해당 서브그리드는 올바르지 않은 행이므로 더 이상의 검사는 불필요하니 반복문을 나간다.
             */
            if (cnt[sudoku[c+y][r+x]] == 0) ++cnt[sudoku[c+y][r+x]];
            else {
                con = false;
                break;
            }
        }
        /* 서브그리드가 올바르지 않다면, 더 이상의 추가 검사는 필요하지 않으므로 반복문을 나간다. */
        if (!con) break;
    }
    valid[2][k] = con;
    pthread_exit(NULL);
...
void check_sudoku(void)
{
     ...
     */
    // 여기를 완성하세요
    struct ROW_COL pos[9];
    for (j = 0; j < 3; ++j) {
        for (i = 0; i < 3; ++i) {
            pos[3*j+i].r = 3 * i;
            pos[3*j+i].c = 3 * j;
            pthread_create(&tid[3*j+i+2], NULL, check_subgrid, (void*) &pos[3*j+i]);
        }
    }
    /*
